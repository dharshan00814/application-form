// NexOps_IQ Careers Client Engine
...
function initApp() {
  ...
  if (!localStorage.getItem('nexops_applications') || JSON.parse(localStorage.getItem('nexops_applications')).length === 0) {
    const mockCandidates = [
      ...
      },
      {
        ...
        why_join: "NexOps_IQ is building the core web tools of the future. I want to secure your nodes before threats target customer accounts.",
      ...
      },
      {
        ...
        why_join: "I love crafting beautiful, responsive user flows that feel incredibly snappy. NexOps_IQ's premium visual language aligns perfectly with my design standards.",
    ...
    localStorage.setItem('nexops_applications', JSON.stringify(mockCandidates));
  }

  // Initialize Mock Project Requests database if needed
  if (!localStorage.getItem('nexops_project_requests') || JSON.parse(localStorage.getItem('nexops_project_requests')).length === 0) {
    const mockRequests = [
      {
        id: "REQ-ECOM98",
    ...
    localStorage.setItem('nexops_project_requests', JSON.stringify(mockRequests));
  }
  let localRequests = JSON.parse(localStorage.getItem('nexops_project_requests') || '[]');
  ...
  let localApplications = JSON.parse(localStorage.getItem('nexops_applications') || '[]');
  ...
  function initiateApplicationFlow(preselectedTrack = "") {
    ...
    // Handle Draft loader
    const savedDraft = localStorage.getItem('nexops_careers_draft');
    if (savedDraft) {
  ...
  function saveDraftOffline(showNotify = true) {
    saveCurrentStepData();
    localStorage.setItem('nexops_careers_draft', JSON.stringify(formData));
    if (showNotify) {
  ...
  btnClearDraft.addEventListener('click', () => {
    if (confirm("Are you sure you want to clear your current application progress? This cannot be undone.")) {
      localStorage.removeItem('nexops_careers_draft');
  ...
  function saveLocalSubmission(payload) {
    ...
    localStorage.setItem('nexops_applications', JSON.stringify(localApplications));
  }
  ...
  function saveLocalRequest(req) {
    ...
    localStorage.setItem('nexops_project_requests', JSON.stringify(localRequests));
  }
